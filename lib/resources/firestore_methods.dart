...
import 'package:wtf01/models/post.dart';
...
class FireStoreMethods {
  ...
  Future<void> followRequest(String uid, String followId) async {
    try {
      DocumentSnapshot snap =
      ...
      if (requests.contains(uid)) {
        ...
        // await _firestore.collection('users').doc(uid).update({
        //   'requests': FieldValue.arrayRemove([followId])
        // });
      } else {
        await _firestore.collection('users').doc(followId).update({
        ...
        // await _firestore.collection('users').doc(uid).update({
        //   'requests': FieldValue.arrayUnion([followId])
        // });
      }
    } catch (e) {
      if (kDebugMode) print(e.toString());
  ...
  Future<void> acceptUser(String uid, String followId) async {
    try {
      DocumentSnapshot snap =
      await _firestore.collection('users').doc(uid).get();
      List requests = (snap.data()! as dynamic)['requests'];
  ...
  Future<void> deleteUserRequest(String uid, String followId) async {
    try {
      DocumentSnapshot snap =
      await _firestore.collection('users').doc(uid).get();
      List following = (snap.data()! as dynamic)['following'];
